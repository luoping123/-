      ...
      <view class="u-tabs__wrapper__scroll-view-wrapper">
        <scroll-view :scroll-x="scrollable" :scroll-left="scrollLeft" scroll-with-animation
          ...
          <view class="u-tabs__wrapper__nav" ref="u-tabs__wrapper__nav" :style="[{
            ...
            <view :class="[item.week?'choose_date':'choose_cate',`u-tabs__wrapper__nav__item-${index}`, item.disabled && 'u-tabs__wrapper__nav__item--disabled']"  :style="[viewStyle(index),{width: item.name && item.name.length*43 + 'rpx'}]" v-for="(item, index) in list" :key="index" @tap="clickHandler(item, index)">
                <text style="padding-left: 5rpx;">{{item.week||item.name}}</text>
                <text style="padding-left: 5rpx;font-size: 32rpx;">{{item.date}}</text>
            </view>
